...
function termsUpdateEmail({ name, termsUrl, effectiveDate = '' }) {
  const firstName = String(name || '').trim().split(/\s+/)[0] || '';
  const url = String(termsUrl || absoluteUrl('/policies'));
  const content = `
    <div style="text-align:center;">
      <div style="font-size:12px;letter-spacing:2.4px;color:#9a7440;font-weight:700;margin-bottom:12px;">עדכון תנאים</div>
      <h1 style="margin:0 0 14px;font-size:30px;line-height:1.2;color:#142333;font-weight:700;">עודכנו תנאי VerSans${firstName ? `, ${escapeHtml(firstName)}` : ''}</h1>
      <p style="margin:0 auto;max-width:490px;font-size:16px;line-height:1.8;color:#56616a;">בוצע עדכון מהותי בתנאי האתר או במדיניות. אפשר לעיין בנוסח המעודכן בקישור שלמטה. השינוי אינו מוחל רטרואקטיבית באופן שפוגע בזכויות שנוצרו לפני העדכון.</p>
      ${button('לעיון בתנאים המעודכנים', url)}
      ${effectiveDate ? `<p style="margin:18px 0 0;font-size:13px;color:#7b858d;">גרסה בתוקף: ${escapeHtml(effectiveDate)}</p>` : ''}
      <p style="margin:20px auto 0;max-width:490px;font-size:13px;line-height:1.8;color:#7b858d;">זהו אימייל שירותי על שינוי בתנאים, ואינו תלוי בהסכמה לקבלת פרסום.</p>
    </div>`;
  return {
    subject: 'עדכון בתנאי VerSans',
    html: emailFrame(content, 'VerSans • versans.com'),
    text: `עודכנו תנאי VerSans${firstName ? `, ${firstName}` : ''}. אפשר לעיין בנוסח המעודכן: ${url}${effectiveDate ? `\nגרסה בתוקף: ${effectiveDate}` : ''}\nהשינוי אינו מוחל רטרואקטיבית באופן שפוגע בזכויות קיימות.`
  };
}
...
function orderConfirmationEmail({ orderRef, customer = {}, items = [], orderTotal = 0, currency = 'ILS' }) {
  ...
  const itemRows = (Array.isArray(items) ? items : []).map((item) => {
    ...
    const safeItemOrderRef = escapeHtml(item.itemOrderRef || orderRef || '');
    ...
    return `<tr>
      <td style="padding:16px 0;border-bottom:1px solid #eee8e2;text-align:right;vertical-align:top;">
        ...
        <div style="margin-top:6px;font-size:11px;color:#7b858d;">מספר הזמנה למוצר</div>
        <div style="margin-top:2px;font:700 12px ui-monospace,SFMono-Regular,Menlo,monospace;color:#142333;direction:ltr;">${safeItemOrderRef}</div>
        ${safeDetails}
      </td>
  ...
  const content = `
    <div style="text-align:right;">
      ...
      <div style="background:#f7f8fa;border:1px solid #e6e9ed;border-radius:12px;padding:14px 16px;margin-bottom:22px;text-align:center;font-size:13px;line-height:1.7;color:#56616a;">לכל מוצר בהזמנה יש מספר VerSans נפרד. שמרו את המספר שמופיע ליד כל מוצר כדי לעקוב אחריו.</div>
  ...
  const textItems = (Array.isArray(items) ? items : []).map((item) => {
    ...
    return `- ${item.productName || item.productId || 'מוצר'} | מספר הזמנה: ${item.itemOrderRef || orderRef || ''}${details ? ` (${details})` : ''} x${Math.max(1, Number(item.quantity) || 1)} - ${moneyLabel(item.lineTotal, currency)}`;
  }).join('\n');

  return {
    subject: 'אישור הזמנה VerSans',
    html: emailFrame(content, 'VerSans • versans.com'),
    text: `ההזמנה שלכם התקבלה בהצלחה. לכל מוצר מספר הזמנה נפרד:\n\n${textItems}\n\nסה״כ: ${moneyLabel(orderTotal, currency)}\n\nפרטי משלוח: ${fullName || '-'}, ${addressLine || '-'}${notes ? `\nהערה: ${notes}` : ''}`
  };
}
