...
async function get17TrackRealTimeInfo(trackingNumber, carrierCode = null, options = {}) {
  // Start with Standard (1 quota). If 17TRACK only returns a cached NotFound /
  // empty event set, escalate once to Instant (10 quotas) on an explicit admin
  // refresh so we do not keep displaying stale "registered" forever.
  const extra = options && typeof options === 'object' ? options : {};
  ...
  let candidates = [];
  try { candidates = candidates.concat(await fetchCandidates(extra.instant ? 1 : 0, true)); } catch (_) {}
  let best = bestTrackingEntry(candidates);
  ...
  if (!entryHasUsefulTracking(best) && !extra.skipAutoCarrierFallback) {
    try { candidates = candidates.concat(await fetchCandidates(extra.instant ? 1 : 0, false)); } catch (_) {}
    best = bestTrackingEntry(candidates);
  }

  // Standard mode can legitimately return a cached empty snapshot for up to
  // three hours. On a manual admin refresh, do one Instant retry only when both
  // scoped and auto-detected Standard queries contain no actual event/status.
  if (!entryHasUsefulTracking(best) && !extra.instant) {
    try { candidates = candidates.concat(await fetchCandidates(1, true)); } catch (_) {}
    best = bestTrackingEntry(candidates);
    if (!entryHasUsefulTracking(best) && !extra.skipAutoCarrierFallback) {
      try { candidates = candidates.concat(await fetchCandidates(1, false)); } catch (_) {}
      best = bestTrackingEntry(candidates);
    }
  }
