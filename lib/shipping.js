...
async function get17TrackRealTimeInfo(trackingNumber, carrierCode = null, options = {}) {
  ...
  const item = trackingQueryItem(trackingNumber, carrierCode, requestOptions);
  const payload = await trackRequest('getRealTimeTrackInfo', [item], 35000);
  let candidates = acceptedTrackingEntries(payload, item.number);
  let best = bestTrackingEntry(candidates);

  // Same multi-carrier fallback as get17TrackInfo, but here an unregistered
  // carrier can still be queried in real time according to the 17TRACK API.
  // This is especially important for AliExpress dropshipping numbers where the
  // label may show a last-mile carrier while Cainiao holds the useful events.
  if (!entryHasUsefulTracking(best) && item.carrier && !extra.skipAutoCarrierFallback) {
    try {
      const autoItem = trackingQueryItem(trackingNumber, null, {
        ...requestOptions,
        skipCarrierInference: true
      });
      const autoPayload = await trackRequest('getRealTimeTrackInfo', [autoItem], 35000);
      candidates = candidates.concat(acceptedTrackingEntries(autoPayload, autoItem.number));
      best = bestTrackingEntry(candidates);
    } catch (_) {}
  }
