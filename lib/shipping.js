...
async function trackRequest(endpoint, items) {
...
async function register17Track(trackingNumber, carrierCode = null, options = {}) {
  ...
  const carrier = normalizeCarrierCode(carrierCode);
  const item = { number };
...
async function get17TrackInfo(trackingNumber, carrierCode = null) {
  ...
  return accepted.find((entry) => normalizeTrackingNumber(entry && entry.number) === item.number) || null;
}
...
function extractTrackingUpdate(entry) {
  ...
  const latestEvent = trackInfo.latest_event && typeof trackInfo.latest_event === 'object' ? trackInfo.latest_event : {};
  ...
  const mainStatus = compact(latestStatus.status, 80);
  const subStatus = compact(latestStatus.sub_status, 120);
