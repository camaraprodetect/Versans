...
test('sitemap contains home, collections, policies, and every product', () => {
  ...
  assert.match(sitemap, /<loc>https:\/\/versans\.com\/policies<\/loc>/);
  for (const product of PRODUCTS) {
    assert.ok(sitemap.includes(`https://versans.com${ROUTES.productPath(product)}`), `missing ${product.id}`);
  }
});
...
test('public pages and products expose canonical metadata', () => {
  ...
  assert.match(read('policies.html'), /<link rel="canonical" href="https:\/\/versans\.com\/policies">/);
  ...
  assert.match(productJs, /https:\/\/versans\.com/);
  assert.match(productJs, /productPath\(product\)/);
});
