...
const p = PRODUCTS.find(x => x.slug === 'product-83');
assert(p, 'product-83 should exist');
assert.strictEqual(p.title.he, 'שעון רויאל דיי - צבע זהב', 'Hebrew title should match');
assert(!p.title.he.includes('-'), 'Hebrew title must not contain an em dash');
assert.strictEqual(p.price, 449.9, 'product-83 price should be 449.90');
assert.strictEqual(p.category, 'watches', 'product-83 should be a watch');
assert.strictEqual(p.colorDisplay, 'image-choice', 'color selector should use image choices');
assert.strictEqual(p.colors.length, 4, 'should have 4 image-based gold color choices');
assert.strictEqual(p.images.length, 4, 'gallery should include the 4 supplied images');
assert.strictEqual(p.cardImage, 'images/products/product-83/product-83-1.png');
assert.strictEqual(p.hoverImage, 'images/products/product-83/product-83-2.png');
...
console.log('product-83 test passed');
